...
float extrution_area, steps_per_cubic_mm_e;
float extruder_advance_k = 0.0, extruder_advance_k2 = 0.0;
...
FORCE_INLINE float max_allowable_speed(float acceleration, float target_velocity, float distance) {
  float i = (target_velocity*target_velocity-2*acceleration*distance);
  if(i < 0.0) i = 0.0;
  return  sqrt(i);
}
...
#ifdef ADVANCE
  long advance_lead_entry = 0, advance_lead_exit = 0, advance_lead_prime = 0, advance_lead_deprime = 0;
  long advance_pressure_relax = 0;

//  if ( block->use_advance_lead )
  {
    long maximum_rate;
    if ( plateau_steps == 0 ) maximum_rate = (max_allowable_speed(-block->acceleration_st, initial_rate, accelerate_steps + 1));
    else maximum_rate = block->nominal_rate;
    if (accelerate_steps > 0)
    {
#ifdef JKN_ADVANCE_LEAD_ACCEL
      advance_lead_entry = (long)(extruder_advance_k * (float)block->acceleration_st);
#else
      advance_lead_entry = (long)(extruder_advance_k * (float)(maximum_rate - initial_rate));
#endif

#ifdef JKN_ADVANCE_LEAD_DE_PRIME
      advance_lead_prime = (long)((extruder_advance_k * (float)initial_rate) - 0.5);
#endif
    ...
    {
#ifdef JKN_ADVANCE_LEAD_ACCEL
      advance_lead_exit = (long)(extruder_advance_k * (float)block->acceleration_st);
#else
      advance_lead_exit = (long)(extruder_advance_k * (float)(maximum_rate - final_rate));
#endif

#ifdef JKN_ADVANCE_LEAD_DE_PRIME
      advance_lead_deprime = (long)((extruder_advance_k * final_rate) - 0.5);
#endif
      if ( extruder_advance_k2 != 0 )
      {
        advance_pressure_relax = (long)((extruder_advance_k2 * (float)block->acceleration_st) / (float)decelerate_steps);
      }
...
#ifdef ADVANCE
  extruder_advance_k  = 0.0001; //extruderAdvanceK;
  extruder_advance_k2 = 0.0001; //extruderAdvanceK2;
//  extrution_area = 0.25 * (noodleDiameter * noodleDiameter) * M_PI;
//  steps_per_cubic_mm_e = axis_steps_per_unit_e/extrution_area;
